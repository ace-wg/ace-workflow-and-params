...
This issue can be solved by having the AS acting as intermediary also for the exchange of C- and RS-generated information, by relying on two new parameters "to_rs" and "from_rs" (see {{sec-more-parameters}}). In particular, C can use "to_rs" for providing the AS with C-generated information, to be relayed to the RS when uploading the access token. Also, the AS can use "from_rs" for providing C with RS-generated information that the AS receives from thge RS as reply to the token uploading.

With reference to the two cases mentioned above, "to_rs" can specify the nonce N1 and C's Recipient ID generated by C, while "from_rs" can specify the nonce N2 and the RS's Recipient ID generated by the RS.
...
* "to_rs" - When using the new ACE workflow, this parameter specifies C-generated information that, according to the profile of ACE used, C has to provide to the RS together with the access token if using the original ACE workflow. This allows the AS to relay such information to the RS upon uploading the access token on behalf of C (see {{sec-open-points-workflow-applicability}}).

  First, C specifies this parameter in the Access Token Request sent to the AS. Then, the AS specifies the conveyed information in the request to the RS sent for uploading the access token on behalf of C. The profile of ACE used has to define the detailed content and semantics of the information specified in the parameter value, and how the AS relays that information to the RS within the request sent to the RS for uploading the access token.

* "from_rs" - When using the new ACE workflow, this parameter specifies RS-generated information that, according to the profile of ACE used, the RS has to provide to C after the uploading of an access token if using the original ACE workflow. This allows the AS to relay such information to C after having uploaded the access token to the RS on behalf of C (see {{sec-open-points-workflow-applicability}}).

  First, the AS receives the RS-generated information in the response from the RS to the request sent for uploading the access token. Then, the AS specifies this parameter in the Access Token Response to C, thereby relaying the information received from the RS. The profile of ACE used has to define the detailed content and semantics of the information specified in the parameter value.
